...
let test1 =
  ...
  in
  (* let queen_b_take = { id = black_queen_id; start_tiles = (Some ('h', 4),
     None); end_tiles = (Some ('h', 5), None); } in let queen_b_take_2 = { id =
     black_queen_id; start_tiles = (Some ('h', 5), None); end_tiles = (Some
     ('g', 6), None); } in let queen_b_take_3 = { id = black_queen_id;
     start_tiles = (Some ('g', 6), None); end_tiles = (Some ('f', 7), None); }
     in let queen_b_take_4 = { id = black_queen_id; start_tiles = (Some ('f',
     7), None); end_tiles = (Some ('e', 7), None); } in let split_knightb_1 = {
     id = black_knight_id1; start_tiles = (Some ('h', 2), None); end_tiles =
     (Some ('g', 0), Some ('g', 4)); } in let merge_knight_b1 = { id =
     black_knight_id1; start_tiles = (Some ('g', 0), Some ('g', 4)); end_tiles =
     (Some ('h', 2), None); } in *)
